...
export class ItemsService {
  ...
  async logging(body: any) {
    ...
    await writeFile(path, body).catch((err) => this.logger.error(err));
  }
  ...
  private async kitDetails(details: CreateDetailsDto[]) {
    ...
    const missingProductFail = () => {
      ...
      this.logger.error(`Missing product code: ${notFoundCodes}`);
      throw new NotFoundException(`Product code ${notFoundCodes} not found`);
    };
